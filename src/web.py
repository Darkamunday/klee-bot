...
<body>
  <div class="card">
    <h1>Klee Admin</h1>
    <p>Sign in with Discord to continue.<br>You must have admin permissions in a server Klee is in.</p>
...
def callback():
    ...
    if not _is_admin_in_bot_guilds(guilds_resp.json()):
        return redirect(url_for("login", error="You don't have admin permissions in any server Klee is in."))
